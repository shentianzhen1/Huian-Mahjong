...
- Player confirmation (2026-09-18, superseding the earlier timing uncertainty): `can_sanjindao = hand_gold_count >= 3`; ordinary complete-hand structure is not required. As soon as the current player's hand has 3+ golds at an applicable decision node, the player may immediately choose Sanjindao or choose PASS/continue. PASS does not consume the hand or force a win; play continues and the hand may develop into other legal outcomes, including the Youjin family. 三金游 and Triple-You (三游) are the same canonical `TRIPLE_YOU` state. 三金倒 remains a separate ×3 outcome, while 三游 is ×16. Sanjindao settlement base, payer, terminal accounting and next dealer remain UNKNOWN.
