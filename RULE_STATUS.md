...
- Sanjindao is a one-shot current-player choice with two confirmed entry forms: **(a)** the opening check after opening flower replacement/open-gold is complete, if the current actor already holds 3+ gold; **(b)** mid-hand when a valid draw changes the player from 2 gold to exactly 3. Sanjindao outranks Qiangjin at that node. The player may declare or PASS/continue; PASS permanently closes Sanjindao for that hand, while ordinary Hu and Youjin-family play remain available. Existing 3/4 gold later does not reopen the window.
...
- **三金倒是一次性“首次3+金/第三金”窗口，不再限定为“刚摸进第3金”。** 两个入口：① 开局补花和开金完成后的首次当前玩家检查，如果此时已经持3张或以上金；② 对局中由2金通过一次有效摸牌变成恰好3金。均提供【三金倒 / 过】，不是强制胡。点【过】后，本局三金倒机会永久关闭；之后仍持3金、再摸第4金，都不会重开。过后3/4金仍可普通自摸/点炮（满足普通结构），也可继续发展游金/双游/三游。match_evidence_002 的回放从首个动作起就清楚显示3张金，随后继续进入游金/双游，直接支持“三金不是强制终局、PASS后特殊路线仍可继续”的目标房行为。每张金仍按1番累计；三金倒仍为独立×3结果，三金游/三游统一为 `TRIPLE_YOU` ×16。三金倒实际结算底数、付款、终局账务和下局庄位仍UNKNOWN。
...
3. Sanjindao remaining gaps: non-flower base, payment, terminal settlement accounting and next dealer. Timing is now: opening current-player check with 3+ gold OR mid-hand 2→3 gold arrival; both are one-shot, DECLARE/PASS optional, PASS permanently closes Sanjindao, and play may continue into ordinary/Youjin-family outcomes. ×3 is confirmed.
