...
- **三金倒的“过”只关闭当前这一次提示，不再视为本局永久关闭。** 已确认入口至少包括：① 开局补花/开金完成后的当前玩家检查，已有3张或以上金时可提示；② 对局中有效摸牌由2金变3金时可提示；③ match_evidence_002 + 玩家本轮确认进一步证明，已经点【过】后，只要后续自己的摸牌节点仍保留恰好3张金，系统可以再次给出【三金倒 / 过】。因此“过一次后本局永不再弹”的旧规则作废。当前录像没有直接证明持4金后的后续摸牌也一定重弹，4金重复窗口先保留UNKNOWN。过三金倒后普通自摸/点炮和游金/双游/三游路线都继续有效；同一录像最终进入游金并以双游×8结算。每张金仍按1番累计；三金倒仍为独立×3结果，三金游/三游统一为 `TRIPLE_YOU` ×16。三金倒实际结算底数、付款、终局账务和下局庄位仍UNKNOWN。
