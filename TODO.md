...
- 已确认待统一实现：花牌1番/张线性累加，4张=4番，春夏秋冬或梅兰竹菊成组不额外叠加；仅八花游的特殊触发/倍率/是否与8个单花番并存仍待确认。本轮不改代码、不跑测试。
- 下一步证据优先级：① 大明杠/暗杠/补杠是否有独立即时杠费、流局是否保留；② 普通数牌碰牌番；③ 多拆解同手牌的取番规则；④ 抢杠胡/杠胡等特殊结算；⑤ 八花游完整结算。
