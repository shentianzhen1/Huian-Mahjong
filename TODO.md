...
- 已确认待统一实现：花牌1番/张线性累加，4张=4番，四花成组不额外叠加；集齐8张花即具备八花游资格，不要求普通牌型，可直接特殊胡，也可选择【过】。若过掉八花游，8张花按基础花番合计8番继续普通胡。项目暂定八花游特殊胡×2（人工设定，非真实证据确认），庄位按正常流程。本轮不改代码、不跑测试。
- 下一步证据优先级：① 大明杠/暗杠/补杠是否有独立即时杠费、流局是否保留；② 普通数牌碰牌番；③ 多拆解同手牌的取番规则；④ 抢杠胡/杠胡等特殊结算。八花游与其他特殊牌型并列时不再单独追问，按统一特殊牌型框架处理。
