...
- Vision 牌面原型：`tiles_v0_1` 已建立首轮真实数据基线并完成两项关键修正。数据来自完整8局真实回放：8段视频、1046×480与960×448两种尺寸、379个去重关键帧、1137个ROI裁剪、168个approved标签、8个独立source_session、34/42个牌类。评测固定采用 `same_region + leave-session-out`，同一录像相邻帧不得跨训练/测试。新增亮色牌面主体归一化与空槽存在检测后，strict静态精确牌面准确率从89.40%提升到**96.03%**：hand_region 126样本/95.24%，draw_region 25样本/100%；类别准确率98.68%。置信阈值0.80时127/151被接受，接受样本准确率98.43%。presence-aware固定窗口稳定性：hand 127/128=99.22%，draw 2/2=100%但样本过少，tile槽合计129/130=99.23%。Gold口径同时修正：当前8局回放左上角只显示黄色牌背marker，不显示具体gold_id；ROI profile现支持 `tile/marker`，marker只报告存在，不输出tile_id。真正gold_id必须在实时开金/翻金事件中识别并写入会话记忆，现有回放不能用于gold牌面准确率。当前仍缺F1/F3/F4/F5/F6/F8/S7/S9，F2/F7仅单session；下一步是扩大draw连续可见稳定性样本、补牌类覆盖、获取从开金前开始的实时录像并增加缩放/移动/遮挡外部验证。所有输出继续固定 `safe_for_executor=false`。证据：`references/vision/2026-09-19/drive_8hand_tiles_v0_1_baseline.md`。
