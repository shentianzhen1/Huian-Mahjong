...
- Vision 牌面原型：`tiles_v0_1` 已从“只有评测基础设施”进入首轮真实数据基线。2026-09-19 直接复用完整8局真实录像，覆盖1046×480与960×448两种录屏尺寸；自动筛出379个去重关键帧并生成1137个 hand/draw/gold ROI 裁剪。首批一轮视觉复核标签168个、40个来源组、34/42个牌类；标签尚需二次复核后才冻结为最终 ground truth。评测口径已改为 **same-region leave-source-group-out**，避免 hand/draw/gold 不同UI渲染域互相污染：当前151个样本可严格评分，精确牌面准确率92.72%；hand_region 126个可评分、93.65%，置信≥0.80的117个为96.58%；draw_region 25个可评分、88.00%。gold_region 当前8个标签均没有“同区域同牌类跨独立来源”的重复样本，因此0个可严格评分，真实准确率为 UNKNOWN / not scorable，不能再写成0%。多帧稳定性首轮只使用6段真正保持同一手牌状态的连续片段：96/96手牌槽位达到≥0.80一致率且实际均100%一致，但多数票只有92/96与人工复核牌面一致，直接出现P4→P2、S4→S3、P2→S2等稳定误识别，进一步验证稳定性不能替代准确率。当前主要缺口为二次复核标签、gold同类跨来源样本、draw/缺失牌类补数、副露后布局和更多录屏条件；所有输出继续固定 `safe_for_executor=false`。证据归档 `references/vision/2026-09-19/room541913_region_aware_baseline.md` 与 `references/vision/2026-09-19/room541913_temporal_stability_pilot.md`。
...
5. Vision首轮真实基线已经建立，不再以“是否有准确率数字”为缺口。下一步优先拆分/归一化 gold_region 与 draw_region 的视觉域，补齐 F1/F3/F4/F5/F6/F8/S7/S9 和 F2/F7跨来源样本，再用同一 leave-source-group-out 口径复测；同时补不同录屏批次、缩放、移动、遮挡。准确率与多帧稳定性未共同达标前不接 Executor。
