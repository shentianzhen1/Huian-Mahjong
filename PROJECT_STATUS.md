...
- Vision 牌面原型：`tiles_v0_1` 已进入首轮真实数据基线。2026-09-19 直接复用完整8局真实录像，覆盖1046×480与960×448两种录屏尺寸；自动筛出379个去重关键帧并生成1137个 hand/draw/gold ROI 裁剪。首批人工审核标签168个，来自8个独立录像 session，覆盖34/42个牌类。评测口径已修正为 `source_session > source_frame > image`，同一录像的相邻帧不能跨训练/测试；真实门槛使用 `same_region + leave-session-out`。严格首轮：151/168可评估（89.88%覆盖），overall 89.40%；hand_region 126样本/93.65%，draw_region 25样本/68.00%，gold_region 当前0样本可严格评估，因为8局没有同一金牌类别跨两个独立session重复。阈值0.80时138/151被接受，接受样本准确率94.20%；hand接受样本96.58%，draw接受样本80.95%。跨区域模板池只作域差异诊断：overall 81.33%、hand 92.91%、draw 54.84%、gold 0%。10连续帧稳定性98.61%（hand 99.22%、draw 87.50%、gold 100%），再次验证“稳定不等于准确/可泛化”。当前Vision最大缺口是draw归一化、gold跨session同类样本、缺失牌类（F1/F3/F4/F5/F6/F8/S7/S9）与独立录屏条件；所有输出继续固定 `safe_for_executor=false`。证据归档 `references/vision/2026-09-19/drive_8hand_tiles_v0_1_baseline.md`。
...
5. Vision首轮真实基线已经建立，不再以“是否有准确率数字”为缺口。下一步优先改善 draw_region；继续录入完整对局以获得 gold_region 同类牌跨独立 session 重复；补齐 F1/F3/F4/F5/F6/F8/S7/S9 和 F2/F7 跨session样本，并始终用 same-region + leave-session-out 复测；同时补不同录屏批次、缩放、移动、遮挡。准确率与多帧稳定性未共同达标前不接 Executor。
