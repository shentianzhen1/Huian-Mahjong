...
- 目标房1张金不能点炮平胡，但标准结构成立时可以自摸；每张花牌基础计1番。八花游现已确认：集齐全部8张花即可直接特殊胡，不要求普通牌型；也可选择【过】继续，过后8花按基础花番合计8番进入普通胡。八花游本身不额外加倍，沿用正常结算规则，庄位也按正常胜负流程处理。
...
3. 普通自摸/点炮的 `HU` 声明已接入，并在 `HU_DECLARED` 保存可复核来源；赢家番数已由 FanAggregator V0.1 自动聚合，只有 `FanResult.complete=True` 才进入真实普通结算。多金番和普通四花组无额外番都已由玩家确认，但代码仍待本批规则收集完后统一实现；当前运行版本仍可能因 `multi_gold_fan` / `flower_groups` 阻断。普通碰牌和拆牌取番歧义仍未解决；八花游特殊结算仍独立UNKNOWN。
4. 三金倒资格口径已被新回放修正：只有“刚摸进第3张金”才出现一次性【声明/过】窗口；点过后本局后续不能仅因仍有3金再次触发。×3倍率和可过继续仍保留；代码当前还是旧3+金通用逻辑，暂不改，待本批规则统一实现/测试。三金游已映射为`TRIPLE_YOU`；游金链4/8/16和`(庄底+赢家番)×倍率`已有+608回归。八花游触发也已补充：集齐8花可直接特殊胡，不要求普通牌型；可过，过后8花=8番继续普通胡。八花游本身无额外倍数，按正常结算与庄位流程处理；与其他特殊状态同时满足时也不设八花游专属例外，统一按特殊牌型通用处理框架执行，因此不再单独保留八花游优先级/重复窗口缺口。抢金窗口归属与PASS关闭规则已接入，但抢金精确胡型及结算仍未完成。杠类抢杠范围已明确：只有补/蓄/加杠 `ADD_KONG` 可被抢，大明杠 `MING_GANG` 和暗杠 `AN_GANG` 不可抢；真实抢杠胡/杠胡/补杠及流局杠费结算继续安全停止。
