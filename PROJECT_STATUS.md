...
- **#69** Public Match Reconstruction：基础契约、observer、Runtime bridge、Action Assembler、中文 Match Ledger、Public Tile Detector 已落地；temporal public-candidate tracker 正把单帧候选升级为稳定轨迹，并通过显式 CandidateChannel 接 identity-UNKNOWN RiverSnapshot。下一阻塞为连续真实帧 channel 校准、误轨迹评估与公开区 identity。
...
3. 推进 #69 Public Match Reconstruction V0.1：10 个真实 bbox 与 geometry detector 已完成；当前把 temporal candidate tracking / CandidateChannel 做稳，并补连续真实帧的误轨迹与 RiverSnapshot 连续性评估，再做公开区 identity；随后把真实 DISCARD/MELD/Youjin/Hu/settlement 喂入现有 observer + assembler + Ledger；冲突保持 UNKNOWN。
