...
3. 普通自摸/点炮的 `HU` 声明已接入，并在 `HU_DECLARED` 保存可复核来源；赢家番数已由 FanAggregator V0.1 自动聚合，只有 `FanResult.complete=True` 才进入真实普通结算。多金番和普通四花组无额外番都已实现；当前运行版本不再因 multi_gold_fan / flower_groups 阻断。普通多拆法现已按“枚举全部合法拆法并取总番最高方案”处理；只有求解枚举被截断时才保留泛化拆法UNKNOWN。普通数牌碰牌番仍未解决；八花游特殊结算使用项目暂定×2。
...
5. 普通局模式使用平胡±1、自摸±2模拟单位，不含真实番数/庄底。该模式的自摸采用能胡即胡策略；不推断游金入口。普通模拟对三金倒和八花游固定走PASS/继续分支：第三金窗口过后允许3金普通自摸，八花PASS后保留8个基础花番。无独立杠费且多拆法已取最高番后，显式游金、3+金点炮、普通数牌碰牌番及抢杠/杠胡特殊结算是当前主要UNKNOWN。
...
2. 普通真实 Settlement + MatchRunner 已接通。无独立杠费后，同口径20场中6/20完整跑完8局；共完成87个真实结算单局，平均4.35局/场。此前UNKNOWN累计中的9个 `decomposition_concealed_triplet_choice` 已按“普通胡取最高总番拆法”规则消除；下一步主要关注3+金点炮、普通数牌碰牌番，以及抢杠胡/杠胡特殊结算。
