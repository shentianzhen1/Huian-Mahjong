...
- 新增真实对局报告`7bc12fa…mp4`：三游+608/-608，当前庄底35，赢家番3（金牌1+两张花2），满足`(35+3)×16=608`。因此花牌每张1番位于胡法倍率内，推翻此前“花水最后单独加”的实现。庄家赢家另×2沿用玩家确认。Rules 的`YoujinScoreTerms`现按`(当前庄底+已审计赢家番)×游倍×庄倍`计算；多金番、付款和下局庄位不猜测。
