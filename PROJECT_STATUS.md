...
4. 三金倒资格口径已被新回放修正：只有“刚摸进第3张金”才出现一次性【声明/过】窗口；点过后本局后续不能仅因仍有3金再次触发。×3倍率和可过继续仍保留；代码当前还是旧3+金通用逻辑，暂不改，待本批规则统一实现/测试。三金游已映射为`TRIPLE_YOU`；游金链4/8/16和`(庄底+赢家番)×倍率`已有+608回归。八花游触发也已补充：集齐8花可直接特殊胡，不要求普通牌型；可过，过后8花=8番继续普通胡。八花游触发/过牌/庄位已确认；倍率方面改为项目人工暂定×2，明确不是目标房真实证据。与其他特殊状态同时满足时不设八花游专属例外，统一按特殊牌型通用处理框架执行。抢金窗口归属与PASS关闭规则已接入，但抢金精确胡型及结算仍未完成。杠类抢杠范围已明确：只有补/蓄/加杠 `ADD_KONG` 可被抢，大明杠 `MING_GANG` 和暗杠 `AN_GANG` 不可抢；真实抢杠胡/杠胡/补杠及流局杠费结算继续安全停止。
