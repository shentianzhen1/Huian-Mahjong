...
- Environment：144 张实体牌守恒、吃碰、PASS、头摸、已解决明杠/暗杠后尾摸、开局及中途补花、16 张零分流局、原子提交、回滚/克隆、死循环保护和合法动作检查。当前内部来源值仍为 `tail`，尚未统一为已确认业务名 `wall_tail`；补杠流程也尚未实现。
...
4. 抢金、三金倒可选动作、二金游/三金游共同触发流程、八花游、游金链、杠胡判定、抢杠、加杠细节、杠分与流局杠分均未实现。三金倒资格与可继续选择、三金游沿用二金游流程、三类杠后尾摸/杠胡分类、八花游存在、花牌基础1番和部分游金对手权限已确认；仍缺少的精确步骤、倍率、叠加与响应窗口继续保持 UNKNOWN。
...
2. 统一摸牌来源枚举为 `wall_head` / `wall_tail`，让明杠、暗杠、补杠复用同一摸牌/补花流程；随后接入统一杠胡分类，抢杠与计分继续保持 UNKNOWN 阻断。
3. 先给胡牌资格增加自摸/点炮来源并实现单金、2金限制，再把普通胡与游金分支拆开；将三金倒建模为 `DECLARE_SANJINDAO` / `CONTINUE_PLAY` 两个合法选择，随后让 AI 比较立即结算与继续追求三金游的价值。
