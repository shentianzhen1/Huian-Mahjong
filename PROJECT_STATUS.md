...
2026-09-18 最新 GitHub Actions 已全绿：FanAggregator→普通真实Settlement V0.1、`MatchProgressState`、`MatchRunner` 和 ordinary-real 8局入口均已接通；无独立杠费与普通胡最高番拆法规则已落地。Core regression 在 Python 3.10 / 3.11 / 3.12 三个版本均各通过206项，0失败、0错误；Legacy baseline advisory 也通过。Vision V0.1 advisory 本轮未手动触发。
...
2026-09-18 普通胡最高番拆法落地后重跑同口径 ordinary-real 基线：10个match seed、RandomAgent/BaselineAgent交换座位共20场，每场目标8局、1000/1000起分。已有12场完整8局；共真实结算118局，平均5.9局/场。多拆解UNKNOWN已全部消失；剩余只有3+金点炮5次、普通数牌碰牌番3次。
...
2. 普通真实 Settlement + MatchRunner 已接通。普通胡最高番拆法落地后，同口径20场中12/20完整跑完8局；共完成118个真实结算单局，平均5.9局/场。当前普通整场的主要规则缺口只剩3+金点炮5次、普通数牌碰牌番3次；随后再补抢杠胡/杠胡等特殊结算。
