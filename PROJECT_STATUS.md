...
2026-09-15 本轮全量自动测试：174 项通过，0 失败、0 跳过（项目136 + Core9 + Environment9 + Recorder14 + Vision6）。新增6项视频证据回归及16项配对/归档复现回归。默认随机回归为10局smoke，100/1000局保留为手动benchmark。

| 工作目录 | 命令 | 结果 |
|---|---|---:|
| 项目根目录 | `python -B -m unittest discover -s tests -v` | 136 通过 |
...
手动评估已核验：seed 0–99交换座位共200局，81局完成（自摸71、点炮2、流局8），119局UNKNOWN（抢杠102、补杠14、三金倒时机3），0步数超限、0循环停止。完整配对仅16组/32局；配对模拟奖励均值Random=-1.6875、Baseline=+1.6875。高UNKNOWN占比使该结果不能代表真实胜率。原始报告保存在本地忽略目录 `data/evaluations/baseline_100_20260915_verified`；已复现第1号记录（seed0交换座位）。
