...
- Vision 牌面原型：`tiles_v0_1` 已从“只有评测基础设施”进入首轮真实数据基线。2026-09-19 直接复用完整8局真实录像，覆盖1046×480与960×448两种录屏尺寸；自动筛出379个去重关键帧并生成1137个 hand/draw/gold ROI 裁剪。首批人工审核标签168个、40个来源组、34/42个牌类，其中166/168可进入跨来源留组评估。leave-source-group-out 首轮精确牌面准确率：overall 84.34%，hand_region 92.91%，draw_region 70.97%，gold_region 0%；类别准确率94.58%。置信阈值0.80时整体接受率92.77%、接受样本准确率87.66%，hand区接受样本准确率96.58%。10连续帧稳定性为98.61%（hand 99.22%、draw 87.50%、gold 100%），同时gold准确率为0%，已验证“时间稳定不等于识别正确”。当前最大Vision缺口改为gold/draw视觉域归一化、缺失牌类（F1/F3/F4/F5/F6/F8/S7/S9）与更多独立录屏条件；所有输出继续固定 `safe_for_executor=false`。证据归档 `references/vision/2026-09-19/drive_8hand_tiles_v0_1_baseline.md`。
