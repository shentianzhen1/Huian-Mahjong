...
- 当前阶段：M2 局中 Environment 已可用；M3 已有 simulation-only 普通局闭环、固定牌墙回归、批量评估及补杠专用抢杠窗口；M4 已建立可解释 BaselineAgent 与 RandomAgent 交换座位回归。真实特殊计分仍安全停止；Vision 保持已有离线原型。
...
- Opening：17/16 发牌、庄家优先分轮补花、骰子开金候选规划；`begin_opening()` 将结果写入 Environment，停在 `OPENING_QIANGJIN_CHECK`。补花按已确认系统交互建模为自动事件，不暴露玩家动作。
- Environment：144张实体牌守恒、吃碰、PASS、头摸、已解决明杠/暗杠后尾摸、开局及中途补花、普通胡声明、普通局16张零分流局、原子提交、回滚/克隆、死循环保护和合法动作检查。摸牌来源为 `wall_head` / `wall_tail`；杠后记录 `kong_kind`、`drawn_tile` 及补花后的有效摸牌。旧 `head` / `tail` 回放仍可迁移。
...
- Simulator：默认真实规则路径停在抢金核验；显式普通局模式跳过该检查，驱动合法动作循环。`enable_added_kong=True` 默认启用补杠专用窗口，设为 False 仅隐藏新候选，不跳过已打开窗口。广义 `enable_rob_kong` 仍默认 False，明杠/暗杠抢杠不在本轮范围。
...
- 明杠、暗杠、补杠完成后均从牌尾摸牌并复用摸花处理，来源为 `wall_tail`；杠牌者随后胡牌统一算杠胡。本轮按用户明确要求接入补杠专用抢杠窗口，实现或单测不作为新增录像证据；明杠/暗杠的可抢范围及所有真实杠胡计分仍未知。
...
2026-09-15 本轮全量自动测试：199项通过，0失败、0跳过（项目161 + Core9 + Environment9 + Recorder14 + Vision6）。本轮新增25项补杠回归，覆盖候选/金牌禁止、PASS/抢杠、连续花、原碰保留、实体牌引用、原子拒绝、配置关闭、Baseline胡牌优先、回放和三个精确UNKNOWN。默认随机回归为10局smoke，100/1000局保留为手动benchmark。
...
本轮手动评估：seed 0–99交换座位共200局，85局完成（自摸74、点炮2、流局9），115局UNKNOWN（明杠/暗杠 `rob_kong` 105、`ADD_KONG_SCORING_UNKNOWN` 7、三金倒时机3），0步数超限、0循环停止。完整配对17组/34局；配对模拟奖励均值Random=-1.6471、Baseline=+1.6471。抢杠胡/杠后胡两个计分UNKNOWN本批随机样本未出现，由固定夹具覆盖。报告位于本地忽略目录 `data/evaluations/added_kong_100_20260915_verified`。补杠窗口实现前的同seed历史结果为81完成、119 UNKNOWN；动作集合已变化，不能把完成数变化解读为真实胜率提升。
...
4. 三金倒资格、×3倍率与声明/继续决策已在 Rules 层实现，三金游已映射为`TRIPLE_YOU`；游金链4/8/16和`(庄底+赢家番)×倍率`已有+608回归。但三金倒动作窗口、二金游/三游状态机、多金番、付款和下局庄位尚未实现。补杠声明/抢杠响应/尾摸已接通；抢金、八花游、明杠/暗杠抢杠范围、真实抢杠胡/杠胡/补杠及流局杠费结算继续安全停止。
...
3. 将已完成的三金倒资格决策接入牌局动作前，先补齐各阶段的准确声明窗口；二金游/三游共同流程、非花底数和付款未确认时继续安全停止。下一批杠相关证据重点是明杠/暗杠的可抢范围、补杠窗口实录、抢杠/杠胡结算和独立杠费。
