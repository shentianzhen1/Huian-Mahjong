...
2026-09-18 新增整场 ordinary-real 基线：10个match seed、RandomAgent/BaselineAgent交换座位共20场，每场目标8局、1000/1000起分并使用真实普通平胡/自摸计分。更新后同口径20场中已有3场完整8局；共真实结算52局，平均2.6局/场。剩余UNKNOWN为杠费9、拆解4、普通碰2、3+金点炮2。该结果说明8局Runner本体已可用，当前瓶颈已从“整场状态机”转移到“尚未确认的番数/杠费证据”。
...
4. 三金倒资格口径已被新回放修正：只有“刚摸进第3张金”才出现一次性【声明/过】窗口；点过后本局后续不能仅因仍有3金再次触发。×3倍率和可过继续仍保留；代码已改为第三金到手的一次性窗口，并已通过回归。三金游已映射为`TRIPLE_YOU`；游金链4/8/16和`(庄底+赢家番)×倍率`已有+608回归。八花游触发也已补充：集齐8花可直接特殊胡，不要求普通牌型；可过，过后8花=8番继续普通胡。八花游触发/过牌/庄位已确认；倍率方面改为项目人工暂定×2，明确不是目标房真实证据。与其他特殊状态同时满足时不设八花游专属例外，统一按特殊牌型通用处理框架执行。抢金窗口归属与PASS关闭规则已接入，但抢金精确胡型及结算仍未完成。杠类抢杠范围已明确：只有补/蓄/加杠 `ADD_KONG` 可被抢，大明杠 `MING_GANG` 和暗杠 `AN_GANG` 不可抢；真实抢杠胡/杠胡/补杠及流局杠费结算继续安全停止。
5. 普通局模式使用平胡±1、自摸±2模拟单位，不含真实番数/庄底。该模式的自摸采用能胡即胡策略；不推断游金入口。普通模拟对三金倒和八花游固定走PASS/继续分支：第三金窗口过后允许3金普通自摸，八花PASS后保留8个基础花番。显式游金、3+金点炮及未解决杠费仍保持UNKNOWN。真实规则路径仍不能完整自动跑局。
...
2. 普通真实 Settlement + MatchRunner 已接通。当前整场级基线（10个match seed×交换座位=20场）中，3/20完整跑完8局；共完成52个真实结算单局，平均2.6局/场。UNKNOWN累计：KONG_FEE_SETTLEMENT_UNKNOWN 9、decomposition_scoring 4、exposed_triplet_fan 2、three_plus_gold_discard_hu 2。下一步首要补独立/流局杠费，其次多拆解、普通碰牌番与3+金点炮。
