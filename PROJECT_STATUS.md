...
- Vision 牌面原型：`tiles_v0_1` 已建立首轮真实数据基线并完成 source_session 防泄漏、亮色牌面主体归一化、空槽存在检测和 same-region 实际推理对齐。数据来自完整8局真实回放：8段视频、1046×480与960×448两种尺寸、379个去重关键帧、1137个ROI裁剪、168个approved标签、8个独立source_session、34/42个牌类。strict静态精确牌面准确率（当前可评分的hand+draw）为**96.03%**：hand_region 126样本/95.24%，draw_region 25样本/100%；类别准确率98.68%，阈值0.80接受样本准确率98.43%。presence-aware固定窗口稳定性：hand 127/128=99.22%，draw 2/2=100%但样本过少。**Gold已纠正**：当前目标房回放左侧黄色高亮牌是正面可见的金牌，可直接读出具体牌面（用户截图可见4筒），因此当前 `gold_region` 必须使用 `tile` 模式直接识别 `gold_id`；此前“黄色牌背marker”判断作废。现有8个gold标签均可作为视觉样本，但由于同一gold类别尚未在两个独立session重复，严格same-region leave-session-out下gold当前仍是0个可评分样本，不能给出泛化准确率。 实验性 Gold V0.1 已加入：SIFT 局部特征屏蔽右上角金字，筒子再用圆点计数二阶段消歧；当前8个已人工确认 gold 样本从普通模板直接识别的6/8提升到8/8，但这只记录为当前样本内原型结果，不能宣称100%泛化。公开状态层后续新增双方玩家面板分数ROI：头像仅作定位锚点，不识别头像/昵称，读取头像下方当前分数并与8局MatchScoreState交叉校验（目标房总分2000守恒）；同时读取剩余牌数和第几局/8等公开信息。当前仍缺F1/F3/F4/F5/F6/F8/S7/S9，F2/F7仅单session；下一步是扩大draw连续可见稳定性样本、获得gold同类跨session重复、补牌类覆盖、加入比分/剩余牌数/局数PublicState识别，并增加缩放/移动/遮挡外部验证。所有输出继续固定 `safe_for_executor=false`。证据：`references/vision/2026-09-19/drive_8hand_tiles_v0_1_baseline.md`。
