...
3. 推进 #69 Public Match Reconstruction V0.1：Hand Context 开局装配已接上；public identity 仍仅 17 标签 / 11 类 / 0 个跨-session同类。下一步优先补 dealer-marker/player-seat 视觉证据与跨-session identity coverage，并继续用连续真实帧验证 stable-channel 误轨迹、actor/turn 与真实 RiverSnapshot；冲突保持 UNKNOWN。
