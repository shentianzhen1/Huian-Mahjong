...
- AI实验淘汰：V0.4“未见张数风险权重”在60场合并测试为29胜31负、平均分差约-9.97，不晋级；V0.5仅在最后3局领先时启用0.25风险权重，两个独立seed区间合计80场约39胜41负、平均分差约-1.85，点炮45 vs 45，同样不晋级。默认前沿仍为V0.3。
- 对手模型研究：第一代“均匀未见暗手→绝对点炮概率”已在100局/2809次弃牌上校准失败：15次真实点炮的平均预测为0%，AUC=0.4993，Brier还差于基础率常数模型，因此明确禁止接入V0.6。第二代改为**听牌条件相对风险分**，只生成公开实体约束下的普通听牌模板，不冒充绝对概率。seed0–99首批100局AUC=0.5628，正样本平均分0.150 vs 负样本0.1285，风险0的397次弃牌无点炮；信号偏弱，正在独立seed held-out验证。
- AI：旧 `BaselineAgent` 保留为稳定对照；`EfficiencyAgent V0.2` 已因固定20 seed×换座40场A/B中仅6胜/33负/1平、平均终分925.125 vs 1074.875而淘汰，不再作为当前前沿。
- AI当前前沿：`ShantenAgent V0.3` 已实现惠安16/17张普通胡向听、有效牌、公开剩余张数与两阶段弃牌排序，不读取对手暗牌或未来牌墙。当前标准整场评估固定20 seed×换座40场全部完成：V0.3 38胜、Baseline 2胜、0平；平均最终分1150.75 vs 849.25，平均分差+301.5；点炮12 vs 43（0.300 vs 1.075次/场），320局胡牌来源为自摸265、点炮55，UNKNOWN=0。旧37:3/+228.25记录仅保留为历史快照。后续AI必须以当前V0.3为比较基线。
...
- Core regression：Python 3.10 / 3.11 / 3.12 **各248项通过，0失败、0错误**。
...
4. AI基准保持ShantenAgent V0.3；8局上下文、点炮统计和标准整场A/B已经接通，V0.4/V0.5均未通过晋级。Issue #6 下一步优先校准公开蒙特卡洛普通点炮概率与运行成本，再把可靠风险概率和真实庄底/比分上下文接入期望得分EV；所有新AI继续直接对V0.3做固定牌墙换座A/B。
