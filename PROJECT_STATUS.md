...
- 抢金窗口规则在 2026-09-18 进一步确认：摸牌/补花/杠结束后的抢金确认只属于当前行动玩家；开局补花和开金完成后同样先检查当前行动玩家。若当前行动玩家选择【放弃】，本回合抢金窗口直接关闭，对手不会接手再次抢金，随后回到该行动玩家应有的正常流程（17张节点出牌，16张节点继续摸牌）。三张或以上金时三金倒在同节点优先展示；`can_sanjindao = hand_gold_count >= 3`。抢金的精确胡型判定、终局/庄位和真实倍率/付款仍未知，代码中的窗口门槛和倍率元数据不得当作结算证据。
...
最近一次有完整记录的全量自动测试仍是 2026-09-15：199项通过，0失败、0跳过（项目161 + Core9 + Environment9 + Recorder14 + Vision6）。2026-09-18 已新增抢金窗口、PASS关闭窗口、16张行动节点以及“明/暗杠不可抢”的针对性回归代码，但远端当前没有 GitHub Actions 状态或工作流运行记录，因此这些新提交尚不能写成“全量测试已通过”。下一次开发提交前应重新跑完整测试并把结果写回本节。
...
2026-09-15 的历史手动评估：seed 0–99交换座位共200局，85局完成（自摸74、点炮2、流局9），115局UNKNOWN（其中当时有105局因明杠/暗杠 `rob_kong` 被阻断、`ADD_KONG_SCORING_UNKNOWN` 7、三金倒时机3），0步数超限、0循环停止。完整配对17组/34局；配对模拟奖励均值Random=-1.6471、Baseline=+1.6471。由于 2026-09-18 已确认并实现“明杠/暗杠不可抢”，这批 UNKNOWN 构成已经过期，只保留作历史基线，必须重新跑同seed评估后才能给出当前完成率。
...
4. 三金倒资格、×3倍率与声明/继续决策已在 Rules 层实现，三金游已映射为`TRIPLE_YOU`；游金链4/8/16和`(庄底+赢家番)×倍率`已有+608回归。抢金窗口归属与PASS关闭规则已接入，但抢金精确胡型及结算仍未完成；八花游完整流程仍未完成。补杠声明/抢杠响应/尾摸已接通；明杠/暗杠不再等待“能否被抢”的证据，但真实抢杠胡/杠胡/补杠及流局杠费结算继续安全停止。
...
3. 抢金下一步只补仍缺的核心：精确胡型/资格判定、实际胡按钮后的终局、倍率/付款和下局庄位；已确认的“当前玩家独占窗口、PASS后不转给对手”不再重复追问。三金倒与游金链继续补齐各阶段声明/升级/取消/付款。杠相关证据重点改为补杠抢杠实录、抢杠胡/杠胡结算和独立杠费，不再追问明杠/暗杠是否可抢。
