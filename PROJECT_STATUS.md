...
- AI当前前沿：`CurrentAgent = TenpaiRiskTieBreakAgent V0.6`。它严格保留V0.3的向听/有效牌总张数/有效牌种类数排序，只在三项完全相同时使用听牌条件相对风险打破最终牌序平局；金牌候选或模型失败时回退V0.3。晋级评估使用300个互不重叠seed pair / 600场完整8局、固定牌墙+换座+Agent身份稳定RNG：V0.6 322胜、V0.3 275胜、3平；平均配对最终分差+18.9633，约95% CI +2.3881～+35.5385；点炮319 vs 350。V0.3继续作为显式对照/消融基线。证据归档 `references/ai/2026-09-19/v06_promotion.md`。
...
- Core regression：Python 3.10 / 3.11 / 3.12 **各288项通过，0失败、0错误**。Legacy Core V0.1.1与Legacy Environment V0.1同样通过。
...
- P1：#6 CurrentAgent V0.6 上的EV/8局上下文；#7 Vision ROI与真实准确率基线。
...
4. AI基准已晋级为 CurrentAgent / TenpaiRisk V0.6；V0.3继续保留为对照。Issue #6 下一步进入可审计EV：把真实庄底、赢家番、胡法倍率、当前比分与剩余局数转为预计收益/损失；UNKNOWN特殊胡不猜倍率。所有新AI先直接对V0.6做固定牌墙+换座+身份稳定RNG整场A/B，并保留V0.3消融对照。
