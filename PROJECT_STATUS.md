...
- 多张真实结算页支持已观察的普通平胡/自摸公式：`(current庄家底 + 赢家番数) × 胡牌倍率`。该结论不能扩展到抢金、三金倒、游金或杠分。
...
- 新增真实对局报告`7bc12fa…mp4`：三游+608/-608，当前庄底35，赢家番3（金牌1+两张花2），满足`(35+3)×16=608`。因此花牌每张1番位于胡法倍率内，推翻此前“花水最后单独加”的实现。庄家赢家另×2沿用玩家确认。Rules 的`YoujinScoreTerms`现按`(current庄底+已审计赢家番)×游倍×庄倍`计算；多金番、付款和下局庄位不猜测。结构化夹具见 `tests/fixtures/settlement_7bc12fa.json`。
...
4. 三金倒资格、×3倍率与声明/继续决策已在 Rules 层实现，三金游已映射为`TRIPLE_YOU`；游金链4/8/16和`(åº门底+赢家番)×倍率`已有+608回归。但三金倒动作窗口、二金游/三游状态机、多金番、付款和下局庄位尚未实现。抢金、八花游、自动杠胡终局、抢杠、补杠、杠分与流局杠分继续安全停止。
