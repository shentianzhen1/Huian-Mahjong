...
- Vision 牌面原型：`tiles_v0_1` 可从 Recorder AVI 抽帧，人工校准 `hand_region` / `draw_region` / `gold_region` 和牌槽，写入可审计 JSONL 标签，并用本地已确认牌块模板离线推理。当前已补齐三层评测基础设施：①按 `source_frame` 整组留出的防泄漏准确率评估；②按 `region+slot` 统计连续帧多数票、一致率和稳定门槛；③ `dataset_status` 在真实评测前审计 approved 标签数、来源组、牌类覆盖、跨来源重复牌类、三个ROI覆盖和可进入留组评测的样本比例。后处理继续覆盖低置信过滤、牌数上限和实体同牌最多四张；所有输出固定 `safe_for_executor=false`。当前仍缺足量真实录像人工标签，因此**尚无可宣称的真实识别准确率数字**。
...
- Vision V0.1 advisory 为手动任务，本轮未触发；Core中新增的Vision评测器/数据审计单测通过，只说明评测基础设施可用，**不代表真实牌面识别准确率已经建立**。
