...
- **#69** Public Match Reconstruction：基础契约、observer、Runtime bridge、Action Assembler、中文 Match Ledger、Public Tile Detector、Candidate Tracker 与 development channel calibration 已落地。公开牌 identity shadow 已否决 hand/draw/global 模板直接复用；独立 `public_action / public_single / public_meld` 标签域已启动，首批 17 个 approved 开发标签仅覆盖 11/34 标准牌且无跨-session同类重复，因此公开 runtime identity 继续 `UNKNOWN`。下一阻塞为扩充 public-region 标签的跨-session覆盖，以及连续真实帧误轨迹 / actor-turn 评估。
...
3. 推进 #69 Public Match Reconstruction V0.1：独立 `public_action / public_single / public_meld` 标签域已建立，但仅 17 个标签 / 11 类 / 0 个跨-session同类；下一步优先扩充跨-session identity coverage，并继续用连续真实帧验证 stable-channel 误轨迹、actor/turn 与真实 RiverSnapshot；冲突保持 UNKNOWN。
