...
- Environment：144 张实体牌守恒、吃碰、PASS、头摸、已解决杠后尾摸、16 张零分流局、原子提交、回滚/克隆、死循环保护和合法动作检查。
...
2026-09-14 全量自动测试：**100 项通过，0 失败，0 跳过**。

| 工作目录 | 命令 | 结果 |
|---|---|---:|
| 项目根目录 | `python -B -m unittest discover -s tests -v` | 73 通过 |
...
2. 中途摸花与杠后摸花仍进入 `NEED_FLOWER_REPLACE` 并停止；仅开局补花已接入。
